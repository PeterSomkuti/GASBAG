...
def plot_map(ax, lon, lat, data, vmin, vmax, gridsize, cmap):
    ...
    binres = sps.binned_statistic_2d(lon, lat, data,
                                     range=[[-180, 180], [-90, 90]],
...
if 'physical_retrieval_results' in f:
    ...
    cont_755 = np.nanmax(f['physical_retrieval_results/modelled_radiance_757nm'][:,0], axis=1)
    cont_772 = np.nanmax(f['physical_retrieval_results/modelled_radiance_771nm'][:,0], axis=1)

    mask_755 = (cont_755 < 3e19) & (f['physical_retrieval_results/757nm_num_iterations'][:,0] < 3)
    mask_772 = (cont_772 < 3e19) & (f['physical_retrieval_results/771nm_num_iterations'][:,0] < 3)

    fs_755 = f['physical_retrieval_results/757nm_SIF_absolute'][:,0]
    fs_755[mask_755] = np.nan

    fs_772 = f['physical_retrieval_results/771nm_SIF_absolute'][:,0]
...
if 'RetrievalResults' in f:
    ...
    cont_755 = f['RetrievalResults/fluorescence_757nm/Ancillary/continuumLevelRadiance'][:,0]
    cont_772 = f['RetrievalResults/fluorescence_771nm/Ancillary/continuumLevelRadiance'][:,0]

    mask_755 = (cont_755 < 3e19)
    mask_772 = (cont_772 < 3e19)
    ...
    fs_755 = f['HighLevelResults/Fluorescence/fluorescence_radiance_757nm'][:,0]
    fs_772 = f['HighLevelResults/Fluorescence/fluorescence_radiance_771nm'][:,0]

fs_rel_755 = fs_755 / ( cont_755 - fs_755 )
fs_rel_755[mask_755] = np.nan
fs_rel_755[np.abs(fs_rel_755) > 0.03] = np.nan
fs_755[np.abs(fs_rel_755) > 0.03] = np.nan
fs_755[np.abs(fs_755) > 3e19] = np.nan
...
gs = gridspec.GridSpec(4, 4, width_ratios=[2,0.5,1,1])
...
plot_map(ax, lon, lat, fs_755, -2e18, 2e18, gridsize, 'BrBG')

ax = plt.subplot(gs[0, 1])
ax.hist(fs_755, range=(-2e18, 2e18), bins=100)
...
ax.hist(100.0 * fs_rel_755, range=(-2, 2), bins=100)
...
ax.hist(fs_772, range=(-1e18, 1e18), bins=100)
...
ax.hist(100.0 * fs_rel_772, range=(-1, 1), bins=100)
